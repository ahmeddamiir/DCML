    ...
    public class DuplicatedData
    {
        ...
        public void PrintDuplicatedData()
        {
            ...
            var indices = Enumerable.Range(0, predictions.Schema.Count)
                ...
                .OrderByDescending(i => scores.GetColumn<float[]>(scores.Schema[i].Name))
                .Take(5);

            Console.WriteLine("Duplicated data points:");
            foreach (var index in indices)
            {
                Console.WriteLine($"{predictions.Schema[index].Name}: {scores.GetColumn<float[]>(scores.Schema[index].Name)}");
            }
        }
